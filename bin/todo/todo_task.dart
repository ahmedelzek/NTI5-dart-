...
Future<Either<String, Map<String, dynamic>>> updateTask(
  ...
  try {
    var updateResponse = await dio.put('tasks/$taskId',
...
Future<Either<String, Map<String, dynamic>>> deleteTask(
  ...
  try {
    var deleteResponse = await dio.delete('tasks/$taskId',
        options: Options(headers: {'Authorization': 'Bearer $accessToken'}));
...
Future<Either<String, String>> addTask(String accessToken) async {
  String newTitle = userInput(validateNonEmpty);
  String newDescription = userInput(validateNonEmpty);
  TaskModel newTask = TaskModel(title: newTitle, description: newDescription);

  try {
    var addResponse = await dio.post('new_task',
...
Future<Either<String, List<TaskModel>>> getTasks(String accessToken) async {
  try {
    var registerResponse = await dio.get('my_tasks',
        options: Options(headers: {'Authorization': 'Bearer $accessToken'}));
